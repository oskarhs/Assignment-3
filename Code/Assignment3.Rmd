...
\mathrm{ACC} := \frac{V}{l}.
$$
Furthermore, we ntoe that the $\mathrm{Accuracy}$ is the maximum likelihood estimate of the hitting probability at level $\bf x^{[1]}$. Using the underlying binomial distribution, we can calculate the estimated standard deviation of the accuracy:
$$
\hat{SD}(\mathrm{ACC}) = \frac{\hat{SD}(V)}{l} = \frac{\sqrt{lp(1-p)}}{l} = \sqrt{\frac{p(1-p)}{l}}.
$$
...
$$
>>>>>>> ec1bccde72c0ced6506d0b3b0de142ef0313996c
